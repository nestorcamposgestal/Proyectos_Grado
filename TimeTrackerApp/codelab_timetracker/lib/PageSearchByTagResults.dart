...
class PageSearchByTagResults extends StatefulWidget {
  int id;
  String tag;

  PageSearchByTagResults(this.id, this.tag);

  @override
  _PageSearchByTagResultsState createState() => _PageSearchByTagResultsState();
}

class _PageSearchByTagResultsState extends State<PageSearchByTagResults> {
  ...
  void initState() {
    ...
    id = widget.id;
    tag = widget.tag;
    futureTree = searchByTag(id, tag);
    _activateTimer();
  }

  void _refresh() async {
    futureTree = searchByTag(id, tag); // to be used in build()
    if (!_disposed)
      setState(() {});
  }
    ...
    return FutureBuilder<Tree>(
      ...
      builder: (context, snapshot) {
        ...
        if (snapshot.hasData) {
          return Scaffold(
              appBar: AppBar(
                title: Text("searchByTagResults").tr(),
