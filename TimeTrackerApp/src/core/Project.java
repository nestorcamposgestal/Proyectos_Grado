...
public class Project extends Element {
    ...
    public Project(JSONObject json_loader) {
        ...
        if (json_loader.has("initialDate")) {
            m_init_DateTime_e = LocalDateTime.parse(json_loader.getString("initialDate"));
        }
        if (json_loader.has("finalDate")) {
            m_end_DateTime_e = LocalDateTime.parse(json_loader.getString("finalDate"));
        }
    ...
    public void setM_element_array(Vector<Element> array) {
        m_element_array = array;
    }
...
// --Commented out by Inspection STOP (03/11/2020 19:29)
    ...
    public JSONObject createJSON() {
        ...
        m_json_project.put("initialDate", Utils.format_localdatetime(m_init_DateTime_e));
        m_json_project.put("finalDate", Utils.format_localdatetime(m_end_DateTime_e));
        m_json_project.put("duration", m_duration_e.toSeconds());
        ...
        if (m_tag_array != null) {
            JSONArray m_json_tags = new JSONArray();

            for (String tag : m_tag_array) {
                m_json_tags.put(tag);
            }

            m_json_project.put("activities", m_json_tags);
        }
    ...
    public JSONObject toJson(int depth) {
        ...
        if (m_tag_array != null) {
            ...
            m_json_project.put("tags", m_json_tags);
        }
...
////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    public Vector<Element> findActiveTasks() {

        Vector<Element> active_array = new Vector<>();
        Vector<Element> active_array_aux;

        if (this.getM_active() == Boolean.TRUE) {
            // active_array.add(this);
            if (m_element_array != null) {
                for (Element element : m_element_array) {
                    if (element.getM_active() == Boolean.TRUE) {
                        if ((element.getM_class().equals("Project")) && (element.getM_element_array() != null)) {
                            active_array_aux = element.findActiveTasks();
                            if (active_array_aux != null) {
                                active_array.addAll(active_array_aux);
                            }
                        } else {
                            if (element.getM_class().equals("Task"))
                                active_array.add(element);
                        }
                    }
                }
            }
    ...
    @Override
    public Element getActiveTasksRoot(Vector<Element> active_tasks) {
        return null;
    }

    public void setActiveTasksRoot(Vector<Element> active_tasks) {

        findActivityById(1).setM_element_array(active_tasks);
    ...
    public void removeElement(Element element) {
        if (element.getM_element_array() != null) {
            Vector<Element> array = element.getM_element_array();
            int index = 0;
            while (index < element.getM_element_array().size()) {
                if (array.get(index).getM_element_array() != null)
                    removeElement(array.get(index));
                else
                    element.getM_element_array().remove(array.get(index));
            }
        }
        element.getM_project().getM_element_array().remove(element);
        element = null;
    }
}
